...
from ...properties.outputs import LargeImageOutput, ImageOutput


@NodeFactory.register("chainner:rest:sd_txt2img_abg")
class StableDiffusionWithABGRemover(AsyncNodeBase):
    def __init__(self):
        super().__init__()
        self.description = ""
        self.inputs = [
    ...
    async def run_async(self, prompt: str, negative_prompt: str, seed: int, steps: int, sampler_name: SamplerName,
                        cfg_scale: float, width: int, height: int, sd_model_checkpoint: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        request_data = {
            "prompt": prompt,
